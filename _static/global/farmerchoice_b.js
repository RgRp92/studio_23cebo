...
this.mkview = function(belief_dat_b, bel_div, results = 0, show_submit = 1) {
	// Make this global to the namespace
	beliefs_ns.belief_dat_b = belief_dat_b;
	beliefs_ns.beldat = belief_dat_b["beldat"];

	let hex_colors = belief_dat_b["hex_colors"];
	let beldat = belief_dat_b["beldat"];
	let tokens = belief_dat_b["tokens"];
	let currency = belief_dat_b["currency"];
	...
	$("#round_number").html(belief_dat_b["round_number"]);
	$("#number_of_rounds").html(belief_dat_b["number_of_rounds"]);

	if (results == 1) {
		$("#pay_round").html(belief_dat_b["pay_round"]);
	}
	...
	for (let i = 1 ; i <= num_bins; i++ ) {
		...
		if (results == 1) {
			$("#bin" + i).val(belief_dat_b["pay_choices"][i - 1]);
		}
		...
		$("#bin" + i).on("input change", function() {
			if (results == 1) {
				...
				set_token_allocations(i, beldat_b)
			}
		});
